  ...
  try {
    ...
    if (!correctAnswer)
      return getErrorResponse(
        401,
        "Não foi encontrado uma resposta correta cadastrada."
      );
    ...
    const { increase, decrease } = POINTS_MAP.get(
      difficulty as WeightValues
    ) || { increase: 0, decrease: 0 };
    const newPoints = isRightAnswer
      ? childPoints + increase
      : childPoints - decrease;
    ...
    try {
      ...
      return getErrorResponse(
        401,
        "Não foi possível atualizar os pontos do usuário."
      );
    }
