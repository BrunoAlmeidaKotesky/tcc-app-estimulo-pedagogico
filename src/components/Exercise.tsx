...
type Props = { data: AnswerByExercise, idx: number, shouldIncrement: boolean }
...
const DifficultyLabel = ({difficulty}: {difficulty: number}) => {
    switch(difficulty) {
        case DIFFICULTY_WEIGHTS.EASY:
            return <span className="bg-green-500 text-white px-2 py-1 rounded-md">Fácil</span>
        case DIFFICULTY_WEIGHTS.MEDIUM:
            return <span className="bg-yellow-500 text-white px-2 py-1 rounded-md">Médio</span>
        case DIFFICULTY_WEIGHTS.HARD:
            return <span className="bg-red-500 text-white px-2 py-1 rounded-md">Difícil</span>;
        case DIFFICULTY_WEIGHTS.VERY_HARD:
            return <span className="bg-red-900 text-white px-2 py-1 rounded-md">Muito Difícil</span>;
        default: return <span className="bg-gray-500 text-white px-2 py-1 rounded-md">Desconhecido</span>;
    }
}